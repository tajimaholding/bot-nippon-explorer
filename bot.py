...
DECOUVERTE = []    # onglet « Découverte » : salons présentés en fin de questionnaire
...
def charger_donnees():
    ...
    global QUESTIONS, CONFIG, INTERETS, ROLES_CONFIG, INVITATIONS, ZONES, DECOUVERTE
    ...
    # --- Onglet "Découverte" (facultatif) : salons présentés en fin d'accueil ---
    decouverte = []
    try:
        for ligne in classeur.worksheet("Découverte").get_all_records():
            salon = str(ligne.get("Salon", "")).strip()
            description = str(ligne.get("Description", "")).strip()
            if salon:
                decouverte.append({"salon": salon, "description": description})
    except gspread.WorksheetNotFound:
        pass
...
def resoudre_salon(serveur: discord.Guild, reference: str):
    """Trouve un salon par ID ou par nom (tolérant : accents, emoji ignorés)."""
    ref = str(reference).strip().lstrip("#")
    if not ref:
        return None
    if ref.isdigit():
        return serveur.get_channel(int(ref))
    ref_normalisee = normaliser_mot(ref)
    for salon in serveur.channels:
        if isinstance(salon, discord.CategoryChannel):
            continue
        if ref_normalisee and ref_normalisee in normaliser_mot(salon.name):
            return salon
    return None
...
class SelecteurQuestion(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        self.view.interaction_finale = interaction  # utilisé par le flux éphémère
...
async def finaliser_questionnaire(utilisateur, serveur, reponses):
    """Étapes communes de fin : rôles, enregistrement, journal. Retourne l'embed de fin."""
    ...
    # 🧭 Salons désormais accessibles (onglet « Découverte »), filtrés selon
    # les accès réels du membre après attribution des rôles
    if DECOUVERTE:
        await asyncio.sleep(1.0)  # laisse les rôles fraîchement attribués se propager
        membre = await obtenir_membre(serveur, utilisateur)
        lignes = []
        for entree in DECOUVERTE:
            salon = resoudre_salon(serveur, entree["salon"])
            if salon is None:
                continue
            if membre is not None and not salon.permissions_for(membre).view_channel:
                continue
            if entree["description"]:
                lignes.append(f"<#{salon.id}> — {entree['description']}")
            else:
                lignes.append(f"<#{salon.id}>")
        if lignes:
            merci.add_field(
                name="🧭 Tes salons, maintenant ouverts",
                value="\n".join(lignes)[:1024],
                inline=False,
            )
...
async def derouler_questionnaire_ephemere(interaction: discord.Interaction, serveur: discord.Guild):
    """Questionnaire en messages éphémères, directement dans le salon.
    Le membre ne quitte pas le serveur ; personne d'autre ne voit rien."""
    utilisateur = interaction.user
    if utilisateur.id in sessions_en_cours:
        await interaction.response.send_message(
            "⏳ Tu as déjà un questionnaire en cours — termine-le d'abord !", ephemeral=True
        )
        return
    if not QUESTIONS:
        await interaction.response.send_message(
            "❌ Aucune question configurée pour le moment. Réessaie plus tard !", ephemeral=True
        )
        return
    sessions_en_cours.add(utilisateur.id)
    try:
        total = len(QUESTIONS)
        courante = interaction  # interaction à laquelle répondre pour la question suivante
        premiere = True
        reponses = []
        for numero, question in enumerate(QUESTIONS, start=1):
            embed = discord.Embed(
            ...
            if numero == 1:
                embed.set_author(name=f"🌸 Bienvenue sur {serveur.name} ! Quelques questions pour t'accueillir…")
            if question["multiple"]:
                embed.set_footer(text="Plusieurs réponses possibles ✔")
            vue = VueQuestion(question, utilisateur.id)
            if premiere:
                await courante.response.send_message(embed=embed, view=vue, ephemeral=True)
                premiere = False
            else:
                await courante.followup.send(embed=embed, view=vue, ephemeral=True)
            await vue.wait()

            if vue.valeurs is None or vue.interaction_finale is None:
                # temps écoulé : le message éphémère n'est plus modifiable, on s'arrête là
                return

            reponses.append(vue.valeurs)
            embed.add_field(name="Ta réponse ✅", value=", ".join(vue.valeurs), inline=False)
            courante = vue.interaction_finale
            try:
                await courante.edit_original_response(embed=embed, view=None)
            except discord.HTTPException:
                pass

        merci = await finaliser_questionnaire(utilisateur, serveur, reponses)
        await courante.followup.send(embed=merci, ephemeral=True)
    finally:
        sessions_en_cours.discard(utilisateur.id)
...
class VueBoutonCommencer(discord.ui.View):
    ...
    async def commencer(self, interaction: discord.Interaction, bouton: discord.ui.Button):
        ...
        if serveur is None:
            ...
            return
        # Questionnaire en messages éphémères, directement ici
...
async def commande_recharger(interaction: discord.Interaction):
    ...
    await interaction.followup.send(
        ...
        f"🧭 Salons de découverte : {len(DECOUVERTE)}",
        ephemeral=True,
    )
...
ccès : aucun, voir, voir-sans-historique,
    voir-reagir, ecrire), commande admin /synchro-salons avec APERÇU puis
    confirmation. Applique les réglages au niveau des CATÉGORIES et
    resynchronise leurs salons, sauf exceptions listées dans Config
    (clé SALONS_EXCEPTIONS). Aucune création ni suppression, jamais.
